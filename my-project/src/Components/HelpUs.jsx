...
const HelpUs = () => {
    ...
    const [fullDonator,setFullDonator]=useState(false);
    const [showTextFur,setShowTextFur]=useState(false);
    const [showTextVol,setShowTextVol]=useState(false);
        ...
        <div className=' bg-black text-white py-10'>
            ...
            <div className='md:flex justify-center items-center w-4/6 mx-auto mt-10 gap-3 '>
                <div className='space-y-3'>
                    ...
                    <p className='text-white'>{fullDonator ? fullTextDonator: previewTextDonator}</p>
                    <button onClick={()=>setFullDonator(!fullDonator)} className='border-2 border-white py-2 px-2 mt-2'> {fullDonator ? 'Show less' : 'Learn More'}</button>
                </div>
                <div className='space-y-3'>
                    <h1 className='text-3xl uppercase'>Funrising</h1>
                    <p className='text-white'>{showTextFur? fullTextFur : previewTextFur}</p>
                   <Link to={'/'} ><button onClick={()=>{setShowTextFur (!showTextFur)}} className='border-2 border-white py-2 px-2 mt-2'>{showTextFur ? 'Show less' : 'Learn More'}</button></Link>
                </div>
                <div className='space-y-3 '>
                    <h1 className='text-2xl uppercase'>Volunteer</h1>
                    <p className='text-white'>{showTextVol ? fullTextVol: previewTextVol}</p>
                    <Link to={'/'} ><button onClick={()=>setShowTextVol(!showTextVol)} className='border-2 border-white py-2 px-2 mt-2'>{showTextVol ? 'Show less' : 'Learn More'}</button></Link>
                </div>
            </div>
        </div>
    );
